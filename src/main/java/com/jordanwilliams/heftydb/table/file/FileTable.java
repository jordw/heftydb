...
public class FileTable implements Table {
  ...
  private class TableIterator implements Iterator<Record> {
    ...
    private Iterator<Long> blockOffsets(Key startKey) throws IOException {
      ...
      return ascending ? recordBlockOffsets.tailSet(startingBlockOffset, false).iterator()
                       : recordBlockOffsets
                 .headSet
                     (startingBlockOffset, false).descendingIterator();
    }
  }
  ...
  private FileTable(long tableId, Paths paths, RecordBlock.Cache recordCache,
