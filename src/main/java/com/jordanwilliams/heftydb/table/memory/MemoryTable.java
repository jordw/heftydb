...
public class MemoryTable implements MutableTable {
  ...
  private final
  ConcurrentNavigableMap<VersionedKey, Record>
      records =
      new ConcurrentSkipListMap<VersionedKey, Record>();
  ...
  public Record get(Key key, long snapshotId) {
    Map.Entry<VersionedKey, Record>
        closestEntry =
        records.floorEntry(new VersionedKey(key, snapshotId));
  ...
  public Iterator<Record> iteratorFrom(Key key, IterationDirection direction, long snapshotId) {
    ...
    Iterator<Record> sourceIterator = direction.equals(IterationDirection.ASCENDING) ?
                                      records.tailMap(versionedKey, true).values().iterator() :
                                      records.headMap(versionedKey).descendingMap().values()
                                          .iterator();
